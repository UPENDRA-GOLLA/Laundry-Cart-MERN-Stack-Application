...
router.post("/signup", async (req, res) => {
  try {
    ...
    if (checkExistingByEmail.length) {
      return res
        .status(400)
        .json(`User With this ${req.body.email} email already exists`);
    }
    if (checkExistingByPhone.length) {
      return res
        .status(400)
        .json(`User With this ${req.body.phone} phone number already exists`);
    }
...
//       const hashedPassword = getUserbyMail.length
//         ? getUserbyMail[0].password
//         : getUserbyPhone[0].password;
//       const isPasswordCorrect = await bcrypt.compare(password, hashedPassword);
//       console.log(isPasswordCorrect);
//       if (isPasswordCorrect) {
//         const user_id = getUserbyMail.length
//           ? getUserbyMail[0]._id
//           : getUserbyPhone[0]._id;
    ...
    if (getUserbyMail.length || getUserbyPhone.length) {
      ...
      if (isPasswordCorrect) {
        const user_id = getUserbyMail.length
          ? getUserbyMail[0]._id
          : getUserbyPhone[0]._id;
        const token = jwt.sign({ id: user_id }, "Upendra");
        res.status(200).json({ user, token });
        console.log("here");
      } else {
        res.status(400).json(`Invalid Password`);
